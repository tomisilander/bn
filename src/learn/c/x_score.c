		    ...
		    suffstat* ss)
{
  pcfg_t* pcfgp;
  ss->nof_pcfgs = 0;
  ss->nof_missing_pcfgs = 0;
  ss->ss = (Pvoid_t) NULL;

  for(pcfgp = pcfgs;  pcfgp < end_pcfgs;  pcfgp += pcfg_length){
    Word_t* freqp;

    if (*pcfgp == MISSING_PCFG) {
      ss->nof_missing_pcfgs += 1;
      continue;
    }
	      ...
	      suffstat* ss)
{
  ...
  serr = gather_parent_configs(dt, nof_parents, parent_ixs, 
				pcfgs, end_pcfgs, pcfg_length);
  CHECKSERR;

  serr = create_freq_map(dt, nof_parents, pcfgs, end_pcfgs, pcfg_length, ss);
  CHECKSERR;

  serr = allocate_memory_for_frequences(dt, vix, ss);
  CHECKSERR;
...
double bde_score_from_ss(suffstat* ss, double ess){
  ...
  /* code missing pcfgs with uniform distribution */
  res -= ss->nof_missing_pcfgs * log(ss->vc_v); 

  return res;
}

static
double fnml_score_from_ss(suffstat* ss){
  ...
  while(freqp < ss->ss_memory_end) {
    ...
    for(; freqp<vfreq_endp; ++freqp){
      if(*freqp){
	pcfreq += *freqp;
	res += (*freqp)*log(*freqp); /* table it */
      }
    }
    res -= pcfreq*log(pcfreq);
    res -= log(reg(pcfreq,ss->vc_v)); /*table it */
  }
  return res;
}

static
double log_ml(suffstat* ss){
  unsigned long* freqp = ss->ss_memory;
...
static
double aic_score_from_ss(suffstat* ss){
  return log_ml(ss) - log(ss->pcc*(ss->vc_v-1));
}

static
double bic_score_from_ss(suffstat* ss, unsigned int N){
  return log_ml(ss) - 0.5*ss->pcc*(ss->vc_v-1)*log(N);
}

/* All the score calls appear similar so */

#define SCOREFBODY(CALL) \
  suffstat ss; \
  int sserr = gather_ss(dt, vix, nof_parents, parent_ixs, &ss);\
  if (sserr != OK) return 0.0; \
  ss.pcc = pcc; \
  double res = CALL; \
  free_ss(&ss); \
  return res
...
double xcat(bde_score)(data* dt, double ess, 
  ...
  SCOREFBODY(bde_score_from_ss(&ss, ess));
}


double xcat(fnml_score)(data* dt, 
			int vix, int nof_parents, int* parent_ixs, double pcc) {
  SCOREFBODY(fnml_score_from_ss(&ss));
}

double xcat(bic_score)(data* dt, 
		       int vix, int nof_parents, int* parent_ixs, double pcc) {
  SCOREFBODY(bic_score_from_ss(&ss, dt->N));
}

double xcat(aic_score)(data* dt, 
		       int vix, int nof_parents, int* parent_ixs, double pcc) {
  SCOREFBODY(aic_score_from_ss(&ss));
}
